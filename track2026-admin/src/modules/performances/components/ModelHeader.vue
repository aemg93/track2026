<template>
  <div class="bg-gradient-to-br from-gray-900 to-gray-950 border border-gray-800 rounded-3xl p-8">

    <div class="flex flex-col lg:flex-row lg:items-center gap-8">

      <!-- AVATAR -->
      <div class="w-28 h-28 rounded-3xl overflow-hidden border border-gray-700 bg-gray-800 flex items-center justify-center shrink-0">

        <img
          v-if="model.profile_photo"
          :src="model.profile_photo"
          :alt="model.nickname || fullName"
...
const initials = computed(() => {
  const first = props.model?.first_name?.charAt(0) || ''
  const last = props.model?.last_name?.charAt(0) || ''
  return (first + last).toUpperCase()
})

const fullName = computed(() =>
  `${props.model?.first_name || ''} ${props.model?.last_name || ''}`.trim()
)
