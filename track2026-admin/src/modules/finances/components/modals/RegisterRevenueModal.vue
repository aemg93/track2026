        ...
        >
            ...
            >
                ...
                <form class="space-y-6 p-8" @submit.prevent="save">
                    <div>
                        <label class="text-sm text-gray-300">Plataforma</label>
                        <select
                    ...
                    <div>
                        <label class="text-sm text-gray-300">Fecha del ingreso</label>
                        <input
                    ...
                    <div>
                        <label class="text-sm text-gray-300">Tipo de ingreso</label>
                        <select
...
const selectedPlatform = computed(() =>
    props.platforms.find(
        platform => Number(platform.id) === Number(form.value.platform_id)
    )
)
...
watch(selectedPlatform, (platform) => {
    ...
    form.value.original_currency =
        platform.type === 'token' ? 'tokens' : 'usd'
})

// 🔎 Debug temporal para confirmar que las plataformas llegan
watch(
    () => props.platforms,
    (value) => {
...
async function save() {
    ...
    try {
        await api.post('/earnings', {
            ...
            earned_at: form.value.earned_at,
            original_amount: Number(form.value.original_amount),
            original_currency: form.value.original_currency,
        })
        emit('saved')
        reset()
        close()
    } catch (error) {
        console.error('ERROR REGISTERING EARNING', error.response?.data ?? error)
    } finally {
        loading.value = false
    }
}
